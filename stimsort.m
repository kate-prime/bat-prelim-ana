    ...
    for i_dep=1:length(folder_dir)
        ...
        for idx_cond=1:2;
            ...
            for i_clust=1:length(file_dir) %loads clusters, changed for split files
                ...
                path=(['W:\Kate\KA001\IC units\',date,'\Matfile\','FT_',depth(4:end)]);
                ch17=load([path,file],'data');
                ch17_FT=ch17.data;
                ...
                for idx_ii=1:2
                    ...
                    elseif ii(idx_ii)==2
                        for iON=1:length(loc_times) %stims that las 100 ms or 50ms make a difference here %KA I think my stims ar 30ms
                            ...
                            stim_onset(1,iON)=loc_times(1,iON)-125; %resized for 30 ms stims, I think, yes %No, FT is not 30ms...
                        end
                    elseif ii(idx_ii)==3
                        for iON=1:length(loc_times) %stims that las 100 ms or 50ms make a difference here %KA I think my stims ar 30ms
                    ...
                    for d=1:size(clusters,2)
                        ...
                        elseif ii(idx_ii)==3
                            if exist([savepath,ch,'_',num2str(d),'_neuron.mat'])
                                ...
                                save([savepath,ch,'_',num2str(d),'_neuron.mat'],'trials_3D','stimon_3D')
                            end
                        end
                    end
                end
